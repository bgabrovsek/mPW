...
mpw_time1 = 'sqrt-mpv-time1-'+str(my_tolerance)+'.csv'
mpw_time2 = 'sqrt-mpv-time2-'+str(my_tolerance)+'.csv'
mpw_time3 = 'sqrt-mpv-time3-'+str(my_tolerance)+'.csv'
mpw_cluster = 'sqrt-mpv-clusters-'+str(my_tolerance)+'.csv'
...
def mpw_single(g, tolerance, result, index):
    ...
    win_size = n
    max_iterations = int(n * math.sqrt(n))
    ...
    while (number_bad_edges[step % win_size] > 0) and (vart >= tolerance) and (step < max_iterations):
...
def mpw_seq_lite(graphs, tol, repeats):
    ...
    for index, g in enumerate(graphs):
        ...
        wr(mpw_cluster, "c"+str(len(set(g.vs['cluster']))))
