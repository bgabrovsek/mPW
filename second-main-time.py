...
mpw_time1 = 's' + int(shft) + '-mpv-time1-'+str(my_tolerance)+'.csv'
mpw_time2 = 's' + int(shft) + '-mpv-time2-'+str(my_tolerance)+'.csv'
mpw_time3 = 's' + int(shft) + '-mpv-time3-'+str(my_tolerance)+'.csv'
mpw_cluster = 's' + int(shft) + '-mpv-clusters-'+str(my_tolerance)+'.csv'
...
def load_graphs(filename=None, n = None, shfty = 0):
    """ Load graphd from files data\filename-****.xml """
    graphs = []
    for counter in range(shfty,n+shfty):
        try:
...
n_samples = [100,100,100,100,100,100,100,20,20,12,8,4]
n_poskusov = 4
...
graphs = load_graphs(filename='graph-' + str(n_v) + '-mu-'+str(mu),n=1,sh=shft)
